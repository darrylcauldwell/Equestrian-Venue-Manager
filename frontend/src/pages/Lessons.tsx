...
export default function Lessons() {
  ...
  const handleCancelRequest = async (requestId: number) => {
    ...
    try {
      await lessonsApi.cancelLesson(requestId);
      loadData();
    } catch (err) {
  ...
  const handleRemoveSchedule = async (scheduleId: number) => {
    ...
    try {
      await lessonsApi.removeRecurringSchedule(scheduleId);
      loadData();
    } catch (err) {
  ...
  const handleRemoveSlot = async (slotId: number) => {
    ...
    try {
      await lessonsApi.removeAvailabilitySlot(slotId);
      loadData();
    } catch (err) {
  ...
  return (
    <div className="lessons-page">
      ...
      {showCoachBookModal && myProfile && (
        <div className="modal-overlay" onClick={() => setShowCoachBookModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Book a Lesson</h2>
