...
export default function AdminCarePlans() {
  ...
  const handleCompletePhase = async (programId: number, phaseId: number) => {
    if (!confirm('Complete this phase and advance to the next?')) return;
    try {
      await rehabApi.completePhase(programId, phaseId);
      const updated = await rehabApi.get(programId);
      setSelectedProgram(updated);
      await loadData();
    } catch (err) {
  ...
  if (loading && programs.length === 0) {
    return (
      <div className="admin-care-plans-page">
        <div className="loading">Loading care plans...</div>
      </div>
    );
  }
        ...
        <div className="modal-overlay" onClick={() => setShowDetailModal(false)}>
          <div className="modal modal-large" onClick={e => e.stopPropagation()}>
                          ...
                          {phase.phase_number === selectedProgram.current_phase &&
                           ...
                           !phase.is_completed && (
                            <button
                              ...
                              onClick={() => handleCompletePhase(selectedProgram.id, phase.id)}
                            >
                              Complete Phase
                            </button>
                          )}
            ...
            <div className="modal-footer">
              ...
              {selectedProgram.status === 'active' && (
                <button className="btn-complete" onClick={() => handleComplete(selectedProgram.id)}>
                  Complete Care Plan
                </button>
              )}
