    ...
    test('staff can open wound care completion modal', async ({ page }) => {
      ...
      if (await woundCareTask.isVisible({ timeout: 2000 })) {
        ...
        if (await completeBtn.isVisible()) {
          ...
          // Modal should appear
          const modal = page.locator('.health-complete-modal, .modal').filter({ hasText: /complete health task/i });
    ...
    test('staff can see submit button in wound care modal', async ({ page }) => {
      ...
      if (await woundCareTask.isVisible({ timeout: 2000 })) {
        ...
        if (await completeBtn.isVisible()) {
          ...
          // Look for submit/complete button in modal
          const submitBtn = page.locator('.modal button').filter({ hasText: /complete.*log|submit/i });
    ...
    test('staff can cancel wound care logging', async ({ page }) => {
      ...
      if (await woundCareTask.isVisible({ timeout: 2000 })) {
        ...
        if (await completeBtn.isVisible()) {
          ...
          // Click cancel button
          const cancelBtn = page.locator('.modal button').filter({ hasText: /cancel/i });
  ...
  test.describe('Step 7: Admin Creates Manual Wound Care Task', () => {
    ...
    test('admin can open create task modal', async ({ page }) => {
      ...
      if (await createBtn.isVisible()) {
        ...
        // Modal should appear
        const modal = page.locator('.modal').filter({ hasText: /report.*task|create/i });
        await expect(modal.first()).toBeVisible();
      }
    });
      ...
      if (await tabSelect.isVisible()) {
        ...
        if (await completedOption.isVisible({ timeout: 500 })) {
          ...
          if (await completedWoundTask.isVisible({ timeout: 1000 })) {
            ...
            const modal = page.locator('.modal').filter({ hasText: /wound care|health/i });
            await expect(modal.first()).toBeVisible();
          }
        }
      }
