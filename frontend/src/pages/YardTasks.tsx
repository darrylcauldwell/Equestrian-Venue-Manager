...
export default function YardTasks() {
  ...
  const [taskToComplete, setTaskToComplete] = useState<YardTask | null>(null);
  ...
  // Forms
  const [newTask, setNewTask] = useState<CreateYardTask>({
    title: '',
  ...
  const handleCompleteTask = async () => {
    ...
    try {
      await tasksApi.complete(taskToComplete.id, completionNotes || undefined);
    ...
    <div className="yard-tasks">
      ...
      <div className="tasks-list">
        ...
        {activeTab === 'scheduled' && (
          (tasks?.scheduled_tasks?.length || 0) === 0 ? (
            <div className="empty-state">No tasks scheduled for future dates</div>
          ) : (
            (() => {
                ...
                .map(([dateStr, dateTasks]) => {
                  ...
                  <div key={dateStr} className={`scheduled-date-group ${isToday ? 'today' : ''}`}>
                    <h3 className="scheduled-date-header">
                      ...
                      <button
                        className="btn-secondary btn-small edit-day-btn"
              ...
              {isAdmin && (
                <>
                  ...
                  <div className="form-group">
                    ...
                    <select
                      value={newTask.assignment_type || 'backlog'}
                      onChange={(e) => setNewTask({ ...newTask, assignment_type: e.target.value as AssignmentType })}
                    >
                    ...
                    <small className="form-help">
                      {newTask.assignment_type === 'specific' && 'Assign to a specific person for a specific day'}
                      {newTask.assignment_type === 'pool' && 'Available for any staff working today'}
                      {newTask.assignment_type === 'backlog' && 'Not yet scheduled - can be assigned to a maintenance day later'}
                    </small>
                  </div>
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => setNewTask({ ...newTask, assigned_to_id: e.target.value ? parseInt(e.target.value) : undefined })}
                      disabled={newTask.assignment_type !== 'specific'}
                    >
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setNewTask({ ...newTask, estimated_cost: e.target.value ? parseFloat(e.target.value) : undefined })}
                      placeholder="0.00"
                    />
          ...
          <div className="modal task-detail-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {selectedTask.status !== 'completed' && selectedTask.status !== 'cancelled' && (
              <div className="task-actions">
                ...
                {isAdmin && (
                  <button className="btn-danger" onClick={() => handleCancelTask(selectedTask.id)}>
                    Cancel Task
                  </button>
                )}
              </div>
            ...
            {selectedTask.status === 'completed' && (isAdmin || selectedTask.assigned_to_id === user?.id) && (
              <div className="task-actions">
                <button className="btn-secondary" onClick={() => handleReopenTask(selectedTask.id)}>
                  Reopen Task
                </button>
              </div>
            )}
      ...
      {showCompleteModal && taskToComplete && (
        <div className="modal-overlay" onClick={() => setShowCompleteModal(false)}>
          <div className="modal task-complete-modal" onClick={(e) => e.stopPropagation()}>
            <h2>Complete Task</h2>
            <div className="task-complete-details">
              <span className={`category-badge category-${taskToComplete.category}`}>
                {enums?.categories.find((c) => c.value === taskToComplete.category)?.label || taskToComplete.category}
              </span>
              <h3>{taskToComplete.title}</h3>
              {taskToComplete.location && (
                <p className="task-location-info">Location: <strong>{taskToComplete.location}</strong></p>
              )}
              {taskToComplete.description && (
                <p className="task-description-info">{taskToComplete.description}</p>
              )}
            </div>
      ...
      {showMaintenanceDayModal && (
        <div className="modal-overlay" onClick={() => setShowMaintenanceDayModal(false)}>
      ...
      {showHealthCompleteModal && healthTaskToComplete && (
        <div className="modal-overlay" onClick={() => setShowHealthCompleteModal(false)}>
          <div className="modal health-complete-modal" onClick={(e) => e.stopPropagation()}>
            <h2>Complete Health Task</h2>
            <div className="health-task-details">
            ...
            {healthTaskToComplete.health_task_type === 'medication' && (
              <div className="health-form">
                <div className="form-group">
                  <label>Was medication given?</label>
            ...
            {healthTaskToComplete.health_task_type === 'wound_care' && (
              <div className="health-form">
                ...
                <div className="form-group">
                  <label>Healing Assessment *</label>
                  <select
                ...
                <div className="form-group">
                  <label>Next Treatment Due</label>
                  <input
                    type="date"
            ...
            {healthTaskToComplete.health_task_type === 'rehab_exercise' && (
              <div className="health-form">
                <div className="form-group">
                  <label>Was exercise completed?</label>
                  <div className="toggle-group">
                    <button
                      type="button"
                ...
                <div className="form-group">
                  <label>Lameness Score (AAEP 0-5)</label>
                  <select
            ...
            <div className="modal-actions">
              <button className="btn-secondary" onClick={() => setShowHealthCompleteModal(false)}>
                Cancel
              </button>
              <button
                className="btn-success"
      ...
      {showEditScheduledModal && editScheduledDate && (
        <div className="modal-overlay" onClick={() => setShowEditScheduledModal(false)}>
          <div className="modal edit-scheduled-modal" onClick={(e) => e.stopPropagation()}>
